/* THE ARCADE CABINET, PORTED FROM snap-hit.online, SECOND PORT.
   ...
   _reference/ is source material, and it is kept off the web by the
   allowlist in .assetsignore rather than by its name. Nothing here loads
   it, links to it or depends on it at runtime.

   WHAT THE SECOND PORT BROUGHT, and why each thing is here.

   1. A LOADING CARD. The attract panel used to be removed the instant
      the iframe was appended, so a slow connection got a black
      rectangle. It now stays up until the frame fires load. It matters
      more here than on snap-hit: the game is one 1.5 MB file and the
      audience is on school wifi.

   2. THE MARQUEE CARRIES THE GAME'S NAME AND IS A LINK. It was the
      studio wordmark, which reads right above four machines on a shelf
      and reads wrong above the single machine in an article.

   3. THE BOTTOM LINK IS AN ARCADE BUTTON reading PLAY DRIFT FEVER,
      rather than an underlined caption reading PLAY IT FULL SIZE.

   4. ONE OBSERVER FOR EVERY CABINET, NEVER UNOBSERVED, and it triggers
      a sweep that judges every frame on its current rectangle instead
      of trusting the record it was handed. See the note above sweep().

   THREE SITE SPECIFIC CHANGES, and only three.
   ...
   2. BOTH WAYS OUT POINT AT "/" IN A NEW TAB. On snap-hit the marquee
      and the button leave for the game's own domain; here that domain
      is the one the reader is already on, so both go to the front page,
      which is the game. They are kept rather than dropped because the
      cabinet is a phone-sized bezel and some people want the whole
      screen without tapping through the machine first.

      NO REFERRAL PARAMETER. snap-hit appends ?from=snaphit so a
      destination's logs can tell a visitor off the shelf from one who
      arrived alone. Here the destination is this same site, so there is
      nothing to tell apart; the game reads only ?debug=1 and ?probe=1,
      so nothing could read it anyway; and a parameter makes a second
      URL for a page whose only job is search placement. Dropped.

   3. NO BAIL BUTTON. snap-hit adds a pill in the opposite corner from
      the close button while a game is full screen, labelled with the
      destination host, because full screen is the moment somebody most
      wants the real thing and the framed copy is not it. Here the
      framed copy IS the real thing, already full screen, so the pill
      would offer a second tab of the game you are playing, next to a
      thumb going for close. Left out on purpose.

   ONE ADDITION, and it is the one behaviour the original does not need.
   snap-hit's cabinets are far apart on a tall shelf and the sweep
   ...
   grew a second one, tap that too. loadInto() parks any other first.

   AND ONE THING KEPT FROM THE FIRST PORT: scrolling back to a cabinet
   that scrolled away brings it back rather than asking for another tap.
   snap-hit never revives. See the note above revive().
...
(function () {
  ...
  var SRC = '/';                 // the game, at the root of this site
  var OUT = '/';                 // where both ways out go, in a new tab
  var NAME = 'Drift Fever';
  var HOLD_MS = 20000;           // the loading card's last resort
  var MARGIN = 120;              // px of slack around the viewport, both ways
  ...
  /* The control panel. Verbatim: a printed overlay with real parts
     ...
     The mark stays. It is a studio credit on the cabinet furniture, the
     same way it belongs on a boot splash, and it is not a route off the
     page: the marquee above it is what carries the game's name now. */
  ...
  var CAB =
    '<div class="cab-body">' +
      '<div class="marquee"><a class="marq" target="_blank" rel="noopener"></a></div>' +
      '<div class="aperture">' +
        '<div class="phone">' +
  ...
  /* One record per cabinet that has ever been tapped:

       el      the iframe
       again   the card currently over the screen, or null when the game
               is showing. It is the same element whether it is standing
               in for a game that has not loaded yet or one that has been
               unloaded, because to a reader those are the same card.
       drop    takes the loading card down, or null when nothing is
               loading. Held on the record so park() can cancel a wait.
       parked  true ONLY when scrolling away is what unloaded it. See
               the note in park(). */
  var frames = [];

  /* ---------------- the card that stays up until the frame loads ----------------
     A heavy game should be the card you just tapped rather than a black
     rectangle. The card is already absolutely positioned over the frame,
     so this only has to decide when it comes down.

     The timer is the last resort, not the plan: if load never arrives
     the card goes anyway, which is exactly the behaviour this replaced,
     rather than leaving somebody tapping a card that cannot answer.

     Both the first tap and the tap that revives an unloaded cabinet come
     through here, because both of them are a wait. */
  function holdUntilLoaded(rec, card) {
    var done = false;
    function drop() {
      if (done) return;
      done = true;
      clearTimeout(timer);
      rec.el.removeEventListener('load', drop);
      if (rec.again === card) rec.again = null;
      if (rec.drop === drop) rec.drop = null;
      card.remove();
    }
    var timer = setTimeout(drop, HOLD_MS);
    rec.drop = drop;
    rec.el.addEventListener('load', drop);
  }
  ...
  function zoom(phone) {
    ...
    makeShrink();
    /* FOCUS IS DELIBERATELY LEFT WHERE IT IS. This used to pull focus to
       the close button, which meant a desktop player's arrow keys stopped
       reaching the game the moment they went full screen, and this game
       drops its steering on blur, so a run died mid flight. The keyboard
       exit is Tab, which carries out of the frame and into the parent,
       and Escape while the parent holds focus. Escape is not bound inside
       the frame on purpose: the game uses it itself. */
  }
  ...
  function unzoom() {
    ...
    zoomed = null;
    if (g) g.focus();
    /* The cabinet may be out of view now that the phone is back in flow.
       Chrome does report that as a change, so this is belt and braces,
       but it keeps the decision here rather than resting on a record
       arriving. Harmless either way: sweep judges by the rectangle, and
       does nothing to a frame that is on screen. */
    sweep();
  }
  addEventListener('keydown', function (e) { if (e.key === 'Escape') unzoom(); });

  /* ---------------- unloading whatever scrolled away ----------------
     ONE OBSERVER FOR EVERY CABINET, AND NOTHING IS EVER UNOBSERVED. A
     frame that was unloaded and then brought back is therefore still
     being watched, which is the whole point: the old code observed per
     frame and unobserved on the way out, so a cabinet played a second
     time ran off screen forever. */
  ...
  /* THE OBSERVER IS ONLY A NUDGE. A record says what changed at the
     moment it was queued, which is not the same as what is true by the
     time it is delivered: a cabinet that scrolled away while another was
     full screen arrives here after the collapse has already brought it
     back on screen. So the record is thrown away unread and every frame
     is judged on its current rectangle.

     Deferred to the next frame, and only ever once per frame. Collapsing
     moves a phone back into flow and the page settles over the frames
     that follow, so a sweep run the instant the class comes off reads a
     rectangle that is already out of date and unloads a cabinet the
     player is looking at.

     Parks first, then revives, so that a revive's loadInto() cannot be
     undone by a park later in the same pass. */
  ...
  function sweep() {
    ...
    requestAnimationFrame(function () { requestAnimationFrame(function () {
      ...
      for (i = 0; i < frames.length; i++) {
        if (offScreen(frames[i].el)) park(frames[i], true);
      }
      for (i = 0; i < frames.length; i++) {
        if (frames[i].parked && !offScreen(frames[i].el)) revive(frames[i]);
      }
    }); });
  }

  /* Unload one cabinet and put its card back. Safe to call at any time
     and as often as you like: it does nothing to a frame that is already
     unloaded, and nothing to the one being played full screen. Guarding
     that single frame is what lets every other cabinet still unload while
     one is expanded.

     PARKED BY THE SWEEP, AND THAT DISTINCTION MATTERS. revive() below
     wakes anything on screen that scrolling parked, and loadInto() also
     unloads a frame when a second cabinet is started. Without the flag
     the two fight: start the second cabinet on a tall screen where both
     are visible, loadInto parks the first, the next sweep sees a visible
     frame that is parked and immediately brings it back, and both end up
     live, which is the state one at a time exists to prevent. Only what
     scrolling parked may scrolling wake.

     A LOADING CARD IS TAKEN DOWN FIRST. Scrolling away from a cabinet
     that is still loading has to leave one card over the screen, not
     two, and the pending timer has to go with it. */
  function park(rec, byScroll) {
    if (!rec.el.getAttribute('src')) return;   // already unloaded
    if (rec.phone === zoomed) return;          // the run being played full screen
    if (rec.drop) rec.drop();
    ...
    again.addEventListener('click', function () {
      /* THE SECOND TAP IS A NO-OP WHILE THE CARD IS UP. The first tap
         set a src and left this card standing as the loading card; a
         second one here would restart the download and leave a second
         wait running. The attract panel's own handler has the same guard
         written as "is there an iframe already", which is the same
         question asked of a screen that has not got one yet. */
      if (rec.el.getAttribute('src')) return;
      rec.el.src = rec.src;
      rec.phone.classList.add('live');
      rec.parked = false;
      holdUntilLoaded(rec, again);
      loadInto(rec);
    });
  ...
  /* SCROLLING BACK BRINGS IT BACK, and snap-hit does not do this. On a
     shelf the reader has moved on to the next machine and a tap to
     restart is right. In an article the reader scrolls back up to the
     thing they were playing, and asking them to tap it again because the
     page moved is a worse answer than reviving it. The card is still put
     back on the way out and still works, so both routes in are live. */
  function revive(rec) {
    rec.parked = false;
    if (rec.el.getAttribute('src')) return;
    rec.el.src = rec.src;
    rec.phone.classList.add('live');
    if (rec.again) holdUntilLoaded(rec, rec.again);
    loadInto(rec);
  }

  /* One at a time. See the note at the top: this is the addition. Parked
     with byScroll false, so no later sweep undoes it. */
  function loadInto(rec) {
    for (var i = 0; i < frames.length; i++) {
      if (frames[i] !== rec) park(frames[i], false);
    }
  }
  ...
  function build(cab) {
    ...
    var marq = cab.querySelector('.marq');
    marq.textContent = NAME;
    marq.href = OUT;
    marq.setAttribute('aria-label', 'Play ' + NAME + ' on its own page');

    var visit = cab.querySelector('.visit');
    visit.href = OUT;
    visit.querySelector('span').textContent = 'PLAY ' + NAME.toUpperCase();
    visit.setAttribute('aria-label', 'Play ' + NAME + ' on its own page');
    ...
    attract.addEventListener('click', function () {
      if (screen.querySelector('iframe')) return;   // the second tap, while the card is up
      ...
      var rec = { el: f, src: SRC, screen: screen, phone: phone,
                  name: NAME, again: null, drop: null, parked: false };
      frames.push(rec);
      /* The attract panel does not go: it becomes the loading card, and
         its own click handler is already the guard against a second tap. */
      holdUntilLoaded(rec, attract);
      loadInto(rec);
      watch(f);
    });
  }
